        ...
        children: [
          ...
          SwitchListTile(
            ...
            onChanged: (value) => _saveSetting(
              context,
              () => controller.setPlaybackSettings(autoAdvance: value),
            ),
          ),
          SwitchListTile(
            title: Text(
              context.isChinese ? '默认显示字幕' : 'Show subtitles by default',
            ),
            value: controller.subtitlesEnabled,
            onChanged: (value) => _saveSetting(
              context,
              () => controller.setPlaybackSettings(subtitlesEnabled: value),
            ),
          ),
          ListTile(
          ...
          ListTile(
            ...
            onTap: () => _notificationSettings(context, controller),
          ),
          ListTile(
            title: Text(
...
Future<void> _notificationSettings(
  BuildContext context,
  AppController controller,
) async {
  ...
  try {
    var preferences = await controller.repository.notificationPreferences();
    final scope = controller.accountScope;
    var busy = false;
    ...
    await showDialog<void>(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, update) => AlertDialog(
          title: Text(context.isChinese ? '通知偏好' : 'Notification preferences'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              for (final key in [
                'marketingInAppEnabled',
                ...
                SwitchListTile(
                  ...
                  onChanged: busy
                      ...
                      : (value) async {
                          update(() => busy = true);
                          try {
                            final saved = await controller.repository
                                .notificationPreferences(update: {key: value});
                            if (context.mounted &&
              ...
              Text(
                context.isChinese ? '订单和安全通知仍会保留。系统推送还需手机授权。' : 'Order and security messages stay enabled. Push also requires device permission.',
              ),
            ],
          ),
          actions: [
...
Future<void> _showLegal(BuildContext context, AppController controller) async {
  try {
    final documents = await controller.repository.legalDocuments(
      controller.locale,
    );
    if (!context.mounted) return;
    await showModalBottomSheet<void>(
        ...
        child: ListView(
          ...
          children: [
            ...
            for (final document in documents)
              ExpansionTile(
                title: Text('${document['title'] ?? document['documentType']}'),
                children: [
                  Padding(
                    padding: const EdgeInsets.all(12),
                    child: SelectableText('${document['bodyMarkdown'] ?? ''}'),
                  ),
                ],
              ),
          ],
