...
export class PlatformInteractionController {
  constructor(
    @Inject(InteractionService)
    private readonly interactions: InteractionService,
  ) {}
  ...
  @RequirePermissions({
    mode: 'read',
    permissions: ['platform.interaction.read'],
    scope: 'platform',
  })
  listModeration(@Query() query: Record<string, unknown>) {
    return this.interactions.listModerationQueue('platform', undefined, query);
  }

  @Post('moderation/:targetType/:targetId/actions')
  @RequirePermissions({
    mode: 'write',
    permissions: ['platform.interaction.manage'],
    scope: 'platform',
  })
  moderate(
    @Param('targetType') targetType: string,
    ...
    return this.interactions.moderate(
      'platform', undefined, targetType, targetId, input,
      staffMetadata('platform', principal, request),
    );
  }
  ...
  listSensitiveWords(@Query() query: Record<string, unknown>) {
    return this.interactions.listSensitiveWords('platform', undefined, query);
  }

  @Post('sensitive-words')
  @RequirePermissions({
    mode: 'write',
    permissions: ['platform.sensitive_word.manage'],
    scope: 'platform',
  })
  createSensitiveWord(
    @Body() input: CreateSensitiveWordInput,
