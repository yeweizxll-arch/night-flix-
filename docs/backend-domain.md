...
- 代理商私有内容由本代理商确认上架，不提交总部审核；总部不处理代理商的评论、举报和敏感词。
- 私有剧状态为草稿 → 已发布 / 待定时上架（内部 approved）→ 已下架。已下架可编辑后重新上架；已发布不可原地编辑。
- 确认上架校验租户归属、版本、封面和全部剧集媒体，并保存不可变快照及代理商审计。
- 手动下架取消该剧及其剧集的旧定时任务，重新上架生成新的任务标识；保留已购权益。
- 历史 pending_review 可由代理商撤回为草稿，不自动上架，不再提供总部审批接口。
- 总部只维护自身公共剧池；各代理商独立选剧、审核和上架。总部紧急下架只作用于自身公共内容。
- 评论、弹幕和举报由代理商治理；只应用本租户敏感词。
